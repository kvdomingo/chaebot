...
class Subscription(commands.Cog):
    ...
    @hourly.command(aliases=['subscribe', 'sub'], help='Subscribe the channel to hourly updates of the selected group')
    ...
    @hourly.command(aliases=['unsubscribe', 'usub'], help='Unsubscribe the channel to any hourly update')
    async def twitter_unsubscribe(self, ctx, group: str):
        group = await group_name_matcher(group)
        channels = group['twitterMediaSubscribedChannels']
        channel = list(filter(lambda x: x['channel_id'] == ctx.channel.id and x['group'] == group['id'], channels))
        if channel:
