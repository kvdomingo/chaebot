...
async def vlive_handler(group: dict):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(
            f"{base_url}/getChannelVideoList", params=payload
        ) as res:
            if res.status >= 400:
            ...
            if latest_vid["videoSeq"] != obj["vlive_last_seq"]:
                ...
                obj, _ = await Api.group(
                    group["id"], "patch", dict(vlive_last_seq=latest_vid["videoSeq"])
                )
                release_timestamp = datetime.strptime(
                    latest_vid["onAirStartAt"], "%Y-%m-%d %H:%M:%S"
                )
