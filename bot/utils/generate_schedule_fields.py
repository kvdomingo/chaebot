...
from common.schemas import Comeback
from common.settings import settings


def extract_unique_dates(dt_strings: list[datetime]) -> list[date]:
    dates = [d.astimezone(settings.DEFAULT_TZ).date() for d in dt_strings]
    return sorted(set(dates))


def generate_schedule_fields(schedule: list[Comeback]) -> dict[str, str]:
    unique_dates = extract_unique_dates([s.date for s in schedule])
    cb_fields_date: dict[date, list[str]] = {d: [] for d in unique_dates}

    for doc in schedule:
        dt: datetime = doc.date.astimezone(settings.DEFAULT_TZ)
        dt_time = dt.strftime("%H:%M")
        if (descriptor := doc.album_type) is not None:
            descriptor = descriptor.title()
        else:
            if (descriptor := doc.release) is not None:
    ...
    for key in cb_fields_date.keys():
        is_today = (
            " `<today>`" if key == datetime.now(settings.DEFAULT_TZ).date() else ""
        )
